import { Component, OnInit, Output, OnDestroy, EventEmitter, TemplateRef, ViewChild } from '@angular/core';
...
export class ConfigurationComponent implements OnInit {
  constructor(
    ...
    private SubscriptionLandingComponent: SubscriptionLandingComponent
  ) { }
  ...
  async ngOnInit(): Promise<void> {
    let id = this.SubscriptionLandingComponent.selectedSubscription.id;
    await this.AllSubscriptionService.getSubscriptions_ById(id).toPromise()
      .then((res) => {
        if (res) {
          this.subs = res;
          this.loadPerms = this.subs.subscriptionFeatures;
          console.log("loaded subs for config", this.loadPerms);
        }
      })
      .catch(
        (err) => {
    ...
    // this.loadPerms = this.SubscriptionLandingComponent.selectedSubscription.subscriptionFeatures;
    this.permsString = this.loadPerms.map((x) => x.feature);
    console.log("Loaded Configuration", this.loadPerms, this.permsString);
    ...
    this.dataService.sharedStepperData.subscribe((stepperData) => {
      this.stepperData = stepperData;
      if (
        this.stepperData.configuration &&
        this.stepperData.configuration.length > 0
      ) {
        this.dataService.setPermData(this.stepperData.configuration);
  ...
  savePermission() {
    let configuration = [];
    configuration = configuration.concat(this.accountingCashUp.subtasks);
    configuration = configuration.concat(this.accountingDeposit.subtasks);
    configuration = configuration.concat(this.accountingReconcilation.subtasks);
    configuration = configuration.concat(this.employeeMain.subtasks);
    configuration = configuration.concat(this.employeeLeaves.subtasks);
    configuration = configuration.concat(this.employeeAttendance.subtasks);
    configuration = configuration.concat(this.employeeRequests.subtasks);
    configuration = configuration.concat(this.employeeShifts.subtasks);
    configuration = configuration.concat(this.employeePayroll.subtasks);
    configuration = configuration.concat(this.suppliers.subtasks);
    configuration = configuration.concat(this.suppliersPurchaseOrder.subtasks);
    configuration = configuration.concat(this.suppliersStockManagement.subtasks);
    configuration = configuration.concat(this.inventroyInvoice.subtasks);
    configuration = configuration.concat(this.reportsAccounting.subtasks);
    configuration = configuration.concat(this.forecasrBudgetting.subtasks);
    configuration = configuration.concat(this.administrationClient.subtasks);
    configuration = configuration.concat(this.administrationRestaurant.subtasks);
    configuration = configuration.concat(this.administrationUser.subtasks);
    configuration = configuration.concat(this.administrationSubscription.subtasks);

    this.dataService.setStepperData({
      ...this.stepperData,
      configuration: configuration,
    });
    //console.log("My Arrays ", configuration);
  ...
  saveconfiguration() {
    const dataObject = [];
    let configurationCheckboxes = this.dataService.getStepperData();
    console.log("configurecheck", configurationCheckboxes);
    configurationCheckboxes
      .filter((x) => x.completed)
      .forEach((element) => {
        let myObj = {
          id: "",
          feature: element.code, //code
          subscriptionFeatureActive: element.completed  //truecheckboxes       
        };
        dataObject.push(myObj);
      });
    console.log("Data obj: ", dataObject);
    this.SubscriptionLandingComponent.selectedSubscription.subscriptionFeatures = dataObject;
    console.log("selectedSubscription", JSON.stringify(this.SubscriptionLandingComponent.selectedSubscription));

    this.AllSubscriptionService.updateConfiguration(this.SubscriptionLandingComponent.selectedSubscription)
      .subscribe((res: any) => {
        //console.log(res);
        console.log("Successfully receieved Updated Configuration")
        this.closePermissions();
        // this.modalService.dismissAll();
        this.dialogMessage = "Configuration Saved Successfully";
        this.dialog.open(this.successDialog);
      },
        (err) => {
          //console.log("Error in Updating Configuration");
          this.dialogMessage = "Error in Saving Configuration";
