...
export class UserPermissionsComponent implements OnInit, OnDestroy {
  ...
  employeePublishedShifts: Task = {
    name: "PUBLISHED SHIFTS",
    completed: false,
    color: "primary",
    code: "HRM_PUB",
    subtasks: [
      { name: "Edit", completed: false, color: "accent", code: "HRM_PUB_ROT" },
    ],
  };
  ...
  administrationUser: Task = {
    ...
    subtasks: [
      ...
      { name: "Edit", completed: false, color: "accent", code: "ADM_USR_EDI" },
      {
        name: "Filter Users",
        completed: false,
        color: "accent",
  ...
  ngOnInit(): void {
    ...
    this.permsString = this.loadPerms.map((x) => x.featureCode);
      // console.log("Loaded User Permissions", this.loadPerms, this.permsString, this.userData.role.name);
    ...
    this.employeeShifts.subtasks  = this.employeeShifts.subtasks.filter((x)=>{
     ...
     this.employeePublishedShifts.subtasks  = this.employeePublishedShifts.subtasks.filter((x)=>{
      return this.subscriptionCodeList.includes(x.code)
    })
    ...
    this.employeePublishedShifts.subtasks?.map((x) => {
      if (this.permsString.indexOf(x.code) != -1) x.completed = true;
    });
  ...
  savePermission() {
    let userPermissions = [];
    userPermissions = userPermissions.concat(this.accountingCashUp.subtasks);

    userPermissions = userPermissions.concat(this.accountingDeposit.subtasks);
    userPermissions = userPermissions.concat(
      this.accountingReconcilation.subtasks
    );
    userPermissions = userPermissions.concat(this.employeeMain.subtasks);
    userPermissions = userPermissions.concat(this.employeeLeaves.subtasks);
    userPermissions = userPermissions.concat(this.employeeAttendance.subtasks);
    userPermissions = userPermissions.concat(this.employeeRequests.subtasks);
    userPermissions = userPermissions.concat(this.employeeShifts.subtasks);
    userPermissions = userPermissions.concat(this.employeePublishedShifts.subtasks);
    userPermissions = userPermissions.concat(this.employeePayroll.subtasks);
    userPermissions = userPermissions.concat(this.suppliers.subtasks);
    userPermissions = userPermissions.concat(
      this.suppliersPurchaseOrder.subtasks
    );
    userPermissions = userPermissions.concat(
      this.suppliersStockManagement.subtasks
    );
    userPermissions = userPermissions.concat(this.inventroyInvoice.subtasks);
    userPermissions = userPermissions.concat(this.reportsAccounting.subtasks);
    userPermissions = userPermissions.concat(this.forecasrBudgetting.subtasks);
    userPermissions = userPermissions.concat(
      this.administrationClient.subtasks
    );
    userPermissions = userPermissions.concat(
      this.administrationRestaurant.subtasks
    );
    userPermissions = userPermissions.concat(this.administrationUser.subtasks);
    userPermissions = userPermissions.concat(
      this.administrationSubscription.subtasks
    );
    //  console.log("My Arerya ", userPermissions);
  ...
  allCompleteemployeePublishedShifts: boolean =false;
  ...
  updateAllCompleteEmployeePublishedShift() {
    this.allCompleteemployeePublishedShifts =
      this.employeePublishedShifts.subtasks != null &&
      this.employeePublishedShifts.subtasks.every((t) => t.completed);
  }
  ...
  someCompleteEmployeePublishedShift() {
    if (this.employeePublishedShifts.subtasks == null) {
      return false;
    }
    return (
      this.employeePublishedShifts.subtasks.filter((t) => t.completed).length > 0 &&
      !this.allCompleteemployeePublishedShifts
    );
  }
  ...
  setAllEmployeePublishedShift(completed: boolean) {
    this.allCompleteemployeePublishedShifts = completed;
    if (this.employeePublishedShifts.subtasks == null) {
      return;
    }
    this.employeePublishedShifts.subtasks.forEach((t) => (t.completed = completed));
  }
